<template>
  <div class="group-details">
    ...
    <div v-if="!is_member" class="join-group-section">
      <h3>申请加入该组</h3>
      <textarea
        v-model="join_request.description"
        placeholder="请输入申请描述，例如您的技能或意图"
      ></textarea>
    ...
    <div class="member-list">
      <div
        v-for="member in group_details.members"
        :key="member.user_id"
        class="member-item"
        :class="{ leader: member.is_leader === 1 }"
        @click="fetchMemberDetails(member.user_id)"
      >
        <span>{{ member.username.slice(-3) }}</span>
        <!-- 踢出按钮，仅对组长可见且不能踢出其他组长 -->
        <button
          v-if="is_leader && member.is_leader !== 1"
          class="kick-button"
        ...
        <form @submit.prevent="transferLeader">
          <div class="form-group">
            ...
            <select id="new-leader" v-model="newLeaderId" required>
              <option
                v-for="member in group_details.members.filter((m) => m.user_id !== current_user_id)"
                :key="member.user_id"
        ...
        <div class="modal">
          ...
          <form>
            ...
            <div class="form-group">
              ...
              <textarea
                id="task-description"
                v-model="taskEditting.description"
                :disabled="reassigning"
                placeholder="请输入任务描述"
              ></textarea>
            </div>
